...
with tabs[5]:
    ...
    if api_key:
        try:
            ...
            # Χρήση του Flash μοντέλου για ταχύτητα και αποφυγή 404
            working_model = 'gemini-1.5-flash'
            
            # 1. Ορισμός μεταβλητών από τα Widgets
            all_names = sorted([d['n'] for d in TEAMS_MAP.values()])
            c1, c2 = st.columns(2)
            
            # Εδώ ορίζουμε τα ονόματα h_t και a_t για να τα βρει το prompt
            h_t = c1.selectbox("Home Team", all_names, key="ai_h_final")
            ...
            extra_notes = st.text_area("🗒️ Σημειώσεις τελευταίας στιγμής:", 
                                      placeholder="Π.χ. Βρέχει, λείπει ο αρχηγός...", 
                                      key="notes_final")
            
            if st.button("ΠΑΤΑ ΝΑ ΠΛΗΡΩΘΕΙΣ", type="primary", key="btn_final"):
                with st.spinner("Ο ΚΟΝΤΟΣ αναλύει τα πάντα (Web Search)..."):
                    
                    # 2. Δημιουργία του Prompt ΜΕΣΑ στο κουμπί (για να έχουν τιμή οι μεταβλητές)
                    # Χρησιμοποιούμε διπλά {{ }} για τα CSS/Table styles και μονά { } για τις μεταβλητές μας
                    advanced_prompt = f"""
...
📤 ΜΟΡΦΗ ΑΠΑΝΤΗΣΗΣ (Ελληνικά, Markdown)
...
**{h_t}:** [Τραυματίες] [Αμφίβολοι]
**{a_t}:** [Τραυματίες] [Αμφίβολοι]

### 📊 Advanced Data & xG Dashboard (Πραγματικά Data)
| Μετρική | {h_t} | {a_t} |
|---------|--------|--------|
| xG (Μέσο όρο) | ... | ... |

### 🏟️ Ιστορικό Μοτίβο Αγώνα #{match_number}
- 2022 (Qatar) — Αγώνας #{match_number}: [Αποτέλεσμα]
- 2018 (Russia) — Αγώνας #{match_number}: [Αποτέλεσμα]
- 📌 Συμπέρασμα μοτίβου: ...

### 🔮 Quantitative Prediction Model
| Κατηγορία | Πρόβλεψη | Πιθανότητα |
|-----------|----------|------------|
| Αποτέλεσμα (1-X-2) | {h_t}/Ισοπαλία/{a_t} | XX% |
| Πιο πιθανό σκορ | X-X | XX% |
| Over 2.5 Goals | Ναι/Όχι | XX% |
| Πέναλτι | Ναι/Όχι | XX% |
| Κόκκινη Κάρτα | Ναι/Όχι | XX% |

### 🎯 Στρατηγικό Συμπέρασμα
[Τελική ανάλυση 3-4 γραμμών]
"""
                    try:
                        # Κλήση της συνάρτησης που ορίσαμε πριν
