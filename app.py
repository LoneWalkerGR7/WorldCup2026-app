...
def load_from_db():
    try:
        ...
        if response.data and len(response.data) > 0:
            ...
            # Migration για παλιά δεδομένα
            for m in data:
                if 'htft' not in m: m['htft'] = "TBD"
                if 'ref' not in m: m['ref'] = "TBD"
                if 'turn' not in m: m['turn'] = "Καμία"
            return data
    except:
        pass
    return None
...
def get_ai_prediction(prompt):
    ...
    # Αποφυγή 404 με δυναμική επιλογή ονόματος
...
with tabs[6]:
    st.markdown("### 🔮 Ο ΚΟΝΤΟΣ ΠΡΟΤΕΙΝΕΙ (Advanced AI Analyst)")
    api_key = st.secrets.get("GEMINI_API_KEY")
    if api_key:
        try:
            genai.configure(api_key=api_key)
            c1, c2 = st.columns(2)
            home = c1.selectbox("Home", all_names, key="ai_h_final")
            away = c2.selectbox("Away", all_names, index=1, key="ai_a_final")
            match_number = st.number_input("Αγώνας #", 1, 104, 1, key="match_no_final")
            ...
            if st.button("ΠΑΤΑ ΝΑ ΠΛΗΡΩΘΕΙΣ", type="primary", key="btn_final"):
                with st.spinner("🔍 Ο ΚΟΝΤΟΣ αναλύει τα πάντα (Web Search)..."):
                    advanced_prompt = f"""
Είσαι ένας elite football analyst, data scientist και quant modeler με απόλυτη εξειδίκευση στο Παγκόσμιο Κύπελλο.
Ακολούθησε αυστηρά τη ΜΕΘΟΔΟΛΟΓΙΑ που περιγράφεται παρακάτω — σκέψου βήμα-βήμα (chain-of-thought) πριν βγάλεις οποιαδήποτε πρόβλεψη. Μεταξύ {home} εναντίον {away}.
...
- Αγώνας #{match_number} | {home} vs {away} | Μουντιάλ 2026
...
## ⚽ {home} vs {away} | Μουντιάλ 2026 — Αγώνας #{match_number}
### 📋 Περιβάλλον Αγώνα
...
"""
                    ans = get_ai_prediction(advanced_prompt)
