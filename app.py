    ...
    for i, gId in enumerate(GROUPS_L):
        with cols_s[i % 3]:
            ...
            for tid in g_team_ids:
                ...
                res.append({"Flag": team['img'], "Team": team['n'], "Pts": pts, "GD": gd, "Y": y})
            df = pd.DataFrame(res).sort_values(by=["Pts", "GD"], ascending=False)
            st.data_editor(df, column_config={"Flag": st.column_config.ImageColumn("🏳️")}, hide_index=True)

with t3:
    all_names = sorted([d['n'] for d in TEAMS_MAP.values()])
    sel_t = st.selectbox("Επιλογή Ομάδας:", all_names)
        ...
        try:
            ...
            if st.button("ΠΑΤΑ ΝΑ ΠΛΗΡΩΘΕΙΣ", type="primary"):
                with st.spinner("Analyzing..."):
                    prompt = f"""
                    You are a top football analyst. Analyze match #{m_no}: {home} vs {away} (World Cup 2026).
                    SEARCH the web for:
                    1. Real-time injury updates and player availability for {home} and {away}.
                    2. Tactical setup and current form.
                    3. History of match #{m_no} in 2022, 2018, 2014.
                    
                    Return analysis in Greek using Markdown.
                    """
                    try:
